...
class Mark extends Model {
    static async getStudentsMarks(schoolId, startYear, endYear, className, classroomNumber, subjectInSemesterId, examTypeId) {
        ...
        const exams = await Exam.findAll({
            where,
            attributes: ['id', 'dateOfExam', 'fullMark'],
    ...
    static async handleGetMarksRequest(req, res) {
        try {
            ...
            const account = await Account.findOne({ where:{siteName: req.params.siteName} })
            const school = await School.findOne({ where:{accountId: account.id} })

            const exams = await Mark.getStudentsMarks(school.id, req.params.startYear,
