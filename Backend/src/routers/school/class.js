...
// delete class
// /alhbd/2020-2021/classes/Preschool
router.delete('/:siteName/:startYear-:endYear/classes/:className', auth(['School']), classBelongsToSchool, async (req, res) => {
    try {
        await req.schoolClass[0].destroy()
        res.send(`${req.params.className} class was deleted.`)
    } catch (e) {
        console.log(e.message)
        res.status(400).send('You can\'t delete this class.')
    }
})
        ...
        try {
            ...
            for (const student of req.body) {
                ...
                const classroom = await Classroom.findByCriteria(req.account.school.id, req.params.startYear,
                    req.params.endYear, className, classroomNumber)
                if (classroom)
