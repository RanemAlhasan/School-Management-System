...
router.get('/schoolNames', async(req, res) => {
    try {
        const names = await School.findAll({ attributes:['schoolName'], required: true, include:{
            association:'account', where:{user:'School'}, attributes:['siteName']
        } })
        res.send(names)
    } catch (e) {
        console.log(e)
        res.status(500).send('Failed to get names.')
    }
})
