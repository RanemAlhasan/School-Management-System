...
class SubjectInSemester extends Model {
    ...
    static async getSemesterSubjects(schoolId, startYear, endYear, className, semester, categoryName, name) {
        ...
        if (!semester){
            ...
            if (className) where['$schoolClass.class.name$'] = className
            if (schoolId) where['$schoolClass.schoolId$'] = schoolId
            
            return SubjectInYear.findAll({
                where,
...
SubjectInSemester.belongsTo(SubjectInYear, { foreignKey: { allowNull: false }, onDelete: 'CASCADE' })
