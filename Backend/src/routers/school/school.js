...
router.post('/schools/signup', async (req, res) => {
    try {
        ...
        school.account.sendMail('Welcome To Schoolink!', 'Have a nice experience as a school administrator in our website!')
...
//get school's info
// /schools/alhbd/info
router.get('/schools/:siteName/info', async (req, res) => {
    try {
        let info = await Account.findOne({
            subQuery: false,
            where: { siteName: req.params.siteName }, attributes: ['email', 'phoneNumber'],
            include: {
                association: 'school', attributes: ['schoolName', 'location', 'facebookPage'], required: true
            }, required: true
        })
        if (!info)
            res.status(404).send('No school that has ' + req.params.siteName + ' site name could be found.')
