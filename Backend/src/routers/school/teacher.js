...
router.post('/:siteName/:startYear-:endYear/teachers/add', auth(['School']), async (req, res) => {
    try {
        ...
        teacher.account.sendMail('Welcome To Schoolink!', 'Have a nice experience as a student in our website!' +
...
router.post('/:siteName/:startYear-:endYear/teachers/addExisting', auth(['School']), async (req, res) => {
    try {
        ...
        await TeacherInYear.create(where)

        account.sendMail('Registration in a New School', `You have been added to ${req.account.school.schoolName} school.`)
...
router.post('/:siteName/:startYear-:endYear/teachers/addInClass', auth(['School']), async (req, res) => {
    try {
        ...
        for (let element of schoolClassIds) {
            ...
            let teacherInClass = await TeacherInClass.findOne({
                where: {
                    teacherInYearId: req.body.teacherInYearId,
                    schoolClassId: element
                }
            })
            if (teacherInClass) return res.status(400).send('Teacher is already added to this class.')

            teacherInClass = await TeacherInClass.create({
                teacherInYearId: req.body.teacherInYearId,
                schoolClassId: element
            })
