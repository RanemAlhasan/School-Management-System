...
import ViewStudentsMark from "./ViewStudentsMark";
...
class ViewMarks extends Form {
  ...
  loadStudentsMarks = async () => {
    ...
    let typeId = examTypes.find((type) => type.name === selectedExamType).id;
    ...
    let students = [];
    // Classroom Marks
    if (this.state.selectedClassroom !== "All")
      students = await getClassroomMarks(
        years,
  ...
  handleSelectClassrooms = (element) => {
