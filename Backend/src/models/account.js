...
class Account extends Model {
            ...
            include: ['school',
                ...
                {association: 'student', include: {association: 'personalInfo', attributes: ['firstName', 'lastName']}},
                {
                    association: 'inLocoParent', include: {
                        association: 'student', attributes: ['id'], include: {
    ...
    async generateAuthToken() {
        ...
        if (account.inLocoParent) {
            ...
            payload.studentFullName = account.inLocoParent.student.personalInfo.firstName + ' ' + account.inLocoParent.student.personalInfo.lastName
        }
    ...
    async sendMail(subject, text) {
        try {
            const transporter = nodemailer.createTransport({
                service: 'Gmail',
                auth: {user: 'schoolink.rar@gmail.com', pass: 'schooLink@hbedo.com'}
            })
...
Account.beforeSave(async (account) => {
        ...
        if (!account.siteName && account.user === 'School')
            errorMessage += 'School must has a site name.\n'

        if (account.siteName && await Account.findOne({where: {siteName: account.siteName}}))
            errorMessage += 'Site name must be unique.\n'
